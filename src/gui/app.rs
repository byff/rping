...
use slint::{Timer, TimerMode, Weak, ModelRc};
...
// TableRow type generated by slint-build
pub type TableRow = slint_generated::TableRow;
...
pub struct PingTestApp {
    ...
    window: Option<Weak<slint_generated::MainWindow>>,
    timer: Option<Timer>,
    sort_column: i32,
...
impl PingTestApp {
    ...
    pub fn set_window(&mut self, window: Weak<slint_generated::MainWindow>) {
        self.window = Some(window);
        self.start_timer();
    }

    fn start_timer(&mut self) {
        let app_self = self as *mut PingTestApp;
    ...
    pub fn stop_ping(&mut self) {
        ...
        self.update_status("已停止".to_string());
            ...
            "xlsx" => {
                match excel::read_excel(&path) {
                    Ok((headers, rows)) => {
                        ...
                        if ip_cols.is_empty() {
                            self.update_status("未在Excel中找到IP列".to_string());
    ...
    pub fn sort_table(&mut self, column: i32) {
        ...
        match column {
            ...
            1 => {
                old_idx.sort_by(|&a, &b| {
                    let ha = self.targets.get(a).map(|t| &t.hostname).unwrap_or(&String::new());
                    let hb = self.targets.get(b).map(|t| &t.hostname).unwrap_or(&String::new());
